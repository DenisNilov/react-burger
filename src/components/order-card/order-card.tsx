...
const OrderCard: FC<IOrderCardsProps> = ({ elementPosition, order }) => {
    ...
    const price = useMemo(() => order.ingredients.reduce((acc, ingredientId) => ingredientId ? acc + ingredients.getIngredientPrice(ingredientId) : acc, 0),
        [ingredients, order])
            ...
            <div className={`mt-6 ${styles.header}`}>
                <div className={styles.ingredientsContainer}>
                    {order.ingredients.slice(0, 6).map((ingredientId, index) =>
                        ingredientId && <div className={styles.ingredientImageContainer} data-count={`+${order.ingredients.slice(6).length}`} key={`${order._id}-${index}-${ingredientId}`}>
                            <img src={ingredients.getIngredientImage(ingredientId)} className={styles.ingredientImage} alt={"Картинка ингредиента"} /></div>)
                    }
                </div>
